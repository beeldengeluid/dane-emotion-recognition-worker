...
class RawAudioTextVideoDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        audio_file = self.fnames_audio[index] #MODIFIED
        ...
        with open(fname_t, 'r') as f:
            words = []
            for line in f:
                words.extend(line.strip().split(' ')) #MODIFIED
    ...
    def collater(self, samples):
        ...
        #print(len(sources))
        #print(sources)
        sizes = [len(s['cpc_logits']) for s in sources] #MODIFIED
        ...
        collated_sources = sources[0]['cpc_logits'].new(len(sources), target_size) #Is this for make the sizes alike #MODIFIED
        padded_amount=[]
        for i, (source, size) in enumerate(zip(sources, sizes)): #MODIFIED
            diff = size - target_size
            #print(diff)
            padded_amount.append(abs(diff))
            #assert diff >= 0
            if diff == 0:
                collated_sources[i] = source['cpc_logits']
            elif diff > 0:
                start = np.random.randint(0, diff + 1)
                end = size - diff + start
                collated_sources[i] = source['cpc_logits'][start:end] #MODIFIED
            else:
                delta = abs(diff)
                #m = torch.nn.ConstantPad1d((0,delta),source[-1]) #left padding with the last element
                m = torch.nn.ConstantPad1d((0,delta),0)
                collated_sources[i] = m(source['cpc_logits']) #MODIFIED
