...
class EmotionPredictionCriterion(FairseqCriterion):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--MELD_predictions', action='store_true', help='Gets a prediction') #MODIFIED
        # fmt: on
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.args.regression_target_mos:
            ...
            if self.args.eval_matric: #MODIFIED
                # emos = ["Neutral","Sadness", "Anger", "Joy", "Surprise", "Fear", "Disgust"]

                # test_preds = logits[0].item()
                # test_truth = targets[0].item()
                # for emo_ind in range(7):
                #     test_preds_i = test_preds[emo_ind]  # Accessing the emo_ind-th element
                #     test_truth_i = test_truth[emo_ind]  # Accessing the emo_ind-th element
                ...
                preds = logits.max(dim=1)[1] #ORIGINAL
                ncorrect=(preds == targets).sum().item()
                ...
                logging_output.update({truth_i : targets[0].item()}) #MODIFIED
                logging_output.update({pred_i : preds[0].item()})#MODIFIED
